...
export default function QuizScreen({ onComplete }: { onComplete?: () => void } = {}) {
  ...
  // Reset quiz state on mount (handles both first-run overlay and post-delete re-entry).
  // Using useEffect instead of useFocusEffect so the quiz doesn't reset when the app
  // returns from background mid-progress.
  useEffect(() => {
    setStep(0);
    setAnswers({});
